...
from html_mailer.api import send_mail_with_images
...
logging.basicConfig(filename='log_massmail', level=logging.DEBUG, maxBytes=20, backupCount=3)
            ...
            if DontSendEntry.objects.has_address(message.to_address):
                ...
                try:
                    ...
                    if message.message_html is None:
                        ...
                        core_send_mail(message.subject, message.message_text, message.from_address,
                                       [message.to_address])
                    else:
                        """ Otherwise, send as HTML. """
                        send_mail_with_images(message)

                    MessageLog.objects.log(message, 1) # @@@ avoid using literal result code
                    message.delete()
                    sent += 1
                except (socket_error, smtplib.SMTPSenderRefused, smtplib.SMTPRecipientsRefused, smtplib.SMTPAuthenticationError, IOError), err:
                    message.defer()
